...
import { loadingItems, marks, loadingBatches } from '@/db/schema';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const allItems = await db.select().from(loadingItems).where(inArray(loadingItems.batchId, batchIds)).all();
    ...
    allItems.sort((a, b) => {
      ...
      return ((a as any).运单号 || '').localeCompare((b as any).运单号 || '');
    });

    const wb = new ExcelJS.Workbook();
    const ws = wb.addWorksheet('装柜明细');
    const headers = ['批次号','日期','唛头','运输方式','运单号','货型','品名','尺寸(cm)','件数(箱)','国内单号','单箱体积','总重量','总体积','单价','需支付总价','订单总价','结算状态','应收'];
