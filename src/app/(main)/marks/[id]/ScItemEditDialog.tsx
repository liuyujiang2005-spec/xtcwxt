  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onClick={() => setOpen(false)}>
      <div className="bg-white rounded-xl p-6 w-80 space-y-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
        <h3 className="font-medium text-sm">编辑费用（体积: {volume.toFixed(2)}m³）</h3>
        ...
        <div className="space-y-1 text-xs text-muted-foreground">
          <p>成本合计: ¥{(parseFloat(cost || '0') * volume).toFixed(2)}</p>
          <p>应收合计: ¥{(parseFloat(receivable || '0') * volume).toFixed(2)}</p>
        </div>
