...
export default function UploadLoadingListPage() {
  ...
  const handleExtract = async () => {
    ...
    try {
      ...
      fetch('/api/ai/verify-items', {
        ...
        body: JSON.stringify({
          items: items.map(i => ({ 品名: i.品名, 计费体积: i.计费体积 || i.单项体积 || i.总体积, 货型: i.货型, 运输方式: i.运输方式, 运单号: i.运单号, 单价: i.单价, 应收: i.应收 })),
          type: 'loading',
        }),
        ...
        setPreview(prev => {
          ...
          // 延迟到下一帧更新 summary，避免在 setState 回调里调 setState
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      ...
      if (importRes.ok) { setResult({ passed: true, msg: `导入成功！共 ${items.length} 条记录` }); setPhase('idle'); timerRef.current = setTimeout(() => router.push('/loading-lists'), 2000); }
