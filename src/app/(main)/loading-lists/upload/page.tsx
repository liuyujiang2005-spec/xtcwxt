      ...
      {phase === 'preview' || phase === 'importing' ? (
        <Card>
          ...
          <CardContent className="space-y-4">
            {summary.abnormalCount > 0 && <div className="p-3 rounded-lg bg-yellow-50 text-yellow-700 text-sm max-h-32 overflow-auto">{abnormalItems.map((item, i) => (<div key={i} className="flex gap-2"><span className="shrink-0">第{item.rowIndex}行 [{item.运单号 || '-'}] {item.唛头 || '-'}：{item.reason}</span></div>))}</div>}
