...
export default async function ExpensesPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const scPending = scItems.filter((i) => i.cost_status === '待支出').reduce((s, i) => s + (i.需支付总价_cents || 0), 0);
  const ldPending = ldItems.filter((i) => i.payment_status === '待支付').reduce((s, i) => s + (i.需支付总价_cents || 0), 0);
  const scPaid = scItems.filter((i) => i.cost_status !== '待支出').reduce((s, i) => s + (i.需支付总价_cents || 0), 0);
  const ldPaid = ldItems.filter((i) => i.payment_status === '已支付').reduce((s, i) => s + (i.需支付总价_cents || 0), 0);
  ...
  return (
    <div className="space-y-6">
          ...
          <Table>
            ...
            <TableBody>
              {Array.from(byType.entries()).map(([type, v]) => (
                <TableRow key={type}><TableCell className="font-medium">{type}</TableCell><TableCell className="text-right">{v.count}</TableCell><TableCell className="text-right">¥{v.CNY.toFixed(6)}</TableCell><TableCell className="text-right">THB {v.THB.toFixed(6)}</TableCell></TableRow>
              ))}
            </TableBody>
          </Table>
        </CardContent>
      ...
      <Card>
        <div className="p-4 border-b"><h2 className="font-semibold">自建费用明细（{allExpenses.length} 条）</h2></div>
        <CardContent className="p-0">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>费用类型</TableHead><TableHead className="text-right">金额</TableHead><TableHead>币种</TableHead>
                <TableHead>供应商</TableHead><TableHead>状态</TableHead><TableHead>创建时间</TableHead>
            ...
            <TableBody>
              {sortedExpenses.map((e) => (
                <TableRow key={e.id}>
                  ...
                  <TableCell className="text-right">¥{e.amountCents.toFixed(6)}</TableCell>
      ...
      {scByMark.size > 0 && (
        <div className="space-y-4">
          ...
          {Array.from(scByMark.entries()).map(([markId, items]) => {
            const pendingTotal = items.filter(i => i.cost_status === '待支出').reduce((s, i) => s + (i.需支付总价_cents || 0), 0);
            const hasPending = items.some(i => i.cost_status === '待支出');
            // Group by 运单号 for merge
              ...
              <Card key={markId}>
                <div className="p-3 border-b flex items-center justify-between bg-muted/30">
                  <div>
                    ...
                    {pendingTotal > 0 && <span className="text-sm text-orange-600 ml-3 font-bold">待付 ¥{pendingTotal.toFixed(6)}</span>}
                  </div>
                  {hasPending && <BatchPayScButton markId={markId} />}
                </div>
                <CardContent className="p-0">
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead>品名</TableHead><TableHead>运输</TableHead><TableHead>货型</TableHead><TableHead>国内单号</TableHead><TableHead>件数</TableHead><TableHead className="text-right">金额</TableHead><TableHead>备注</TableHead>
      ...
      {ldByMark.size > 0 && (
        <div className="space-y-4">
          ...
          {Array.from(ldByMark.entries()).map(([markId, items]) => {
            const pendingTotal = items.filter(i => i.payment_status === '待支付').reduce((s, i) => s + (i.需支付总价_cents || 0), 0);
            ...
            return (
              <Card key={markId}>
                <div className="p-3 border-b flex items-center justify-between bg-muted/30">
                  ...
                  {hasPending && <BatchPayLdButton markId={markId} />}
                </div>
                <CardContent className="p-0">
                  <Table>
                    <TableHeader>
                      <TableRow>
                        <TableHead>品名</TableHead><TableHead>运输</TableHead><TableHead>货型</TableHead><TableHead>国内单号</TableHead><TableHead className="text-right">金额</TableHead><TableHead>备注</TableHead>
                        <TableHead>状态</TableHead><TableHead className="text-right">操作</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
