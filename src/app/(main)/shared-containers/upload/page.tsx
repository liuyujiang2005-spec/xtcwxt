      ...
      {phase === 'preview' || phase === 'importing' ? (
        <Card><CardHeader><CardTitle>预览数据 <span className="ml-2 text-sm font-normal text-muted-foreground">共 {summary.totalItems} 条{summary.abnormalCount > 0 ? `，${summary.abnormalCount} 条异常` : '，全部通过'}</span></CardTitle></CardHeader>
          <CardContent className="space-y-4">
            {summary.abnormalCount > 0 && <div className="p-3 rounded-lg bg-yellow-50 text-yellow-700 text-sm max-h-32 overflow-auto">{abnormalItems.map((item, i) => (<div key={i}><span>第{item.rowIndex}行：{item.reason}</span></div>))}</div>}
            <div className="border rounded-lg overflow-auto max-h-80"><Table>
              ...
              <TableBody>{preview.map((item, i) => (
                <TableRow key={i}>
                  ...
                  <TableCell className="text-right">{item.总体积 || '-'}</TableCell><TableCell className="text-right">{item.总重量 || '-'}</TableCell><TableCell className="text-right">{item.计费体积 || '-'}</TableCell><TableCell className="text-right">{item.总计费体积 || '-'}</TableCell>
                  ...
                  <TableCell className="text-xs max-w-[80px] truncate">{item.备注 || '-'}</TableCell><TableCell>{item.结算状态 || '-'}</TableCell><TableCell>{item.柜号 || '-'}</TableCell>
