...
const FONT = '宋体';
const B = { top: { style: 'thin' as const }, left: { style: 'thin' as const }, bottom: { style: 'thin' as const }, right: { style: 'thin' as const } };
...
export async function generateBillXlsx(
  ...
  const bh = (c1: number, c2: number, text: string) => {
    ...
    for (let c = c1; c <= c2; c++) ws.getCell(br, c).border = B;
  };
  bh(1, 2, bankHdrCny);
  bh(3, 4, bankHdrThb);
  bh(5, 10, bankHdrWx);
  ...
  const bv = (c1: number, c2: number, text: string) => {
    ...
    for (let c = c1; c <= c2; c++) ws.getCell(bd, c).border = B;
  };
  bv(1, 2, bankCny);
  bv(3, 4, bankThb);
  bv(11, 14, bankPublic);
